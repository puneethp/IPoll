...
public class UserServ {
	...
	public boolean writeWardUser(String username,Long wardid)
	{
		user = dao.getUser(username);
		if(user!=null)
			if(user.getType().compareToIgnoreCase(User.NORMAL)==0&&wudao.getWardUser(wardid)==null)
				if((user = wudao.updateWardUser(user, new WardUser(username,wardid)))!=null)
					return true;
		return false;
	}
