...
<h2>Requests for Candidate</h2>
<c:if test="${empty requestScope.reqForCandidatureWard}">
	Nothing left !!
</c:if>
<c:forEach var="item" items="${requestScope.reqForCandidatureWard}">
	<div class="span12 well well-small">
		...
		<div class="span12">
			<div class="row-fluid">
				<h5 class="span10">${item.key.reqquestion}
					${item.value.candidateId}</h5>

				<c:if test="${item.key.reqtype eq \"candidature\"}">
					<form name="" action="./viewCandidatureWardAction.action"
						method="post">
						<input type="hidden" value="${item.key.reqid}" name="reqid"
							id="reqid" /> <input type="hidden"
							value="${item.value.candidateId}" name="candidateid"
							id="candidateid" /> <input type="submit" class="span2
							btn pull-right" value="view" />
					</form>
				</c:if>
			</div>
		</div>
