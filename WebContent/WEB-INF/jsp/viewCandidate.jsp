...
<c:if test="${not empty requestScope.candidatecard}">
	<c:if test="${not empty requestScope.votercard}">
		...
		<c:if test="${requestScope.candidatecard.validity eq 3 or requestScope.candidatecard.validity eq 0 or requestScope.candidatecard.validity eq 2 }">
			<form action="./ReportForCandid" method="post">
				<input type="hidden" id="candidateid"
								value="${requestScope.candidatecard.user}" name="userid" />
								<input type="submit" class="btn" value="download">
			</form>
		</c:if>

		<c:if test="${requestScope.candidatecard.validity eq 3 or requestScope.candidatecard.validity eq 0 or requestScope.candidatecard.validity eq 2 	 }">
			<form action="./updateCandidatureReg.action" method="post">
